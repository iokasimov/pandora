...
import Pandora.Pattern.Functor.Applicative (Semimonoidal_)
...
class Covariant_ t source target => Traversable t source target where
	(<<-) :: (Covariant_ u source target, Pointable u target, Semimonoidal_ u target (:*:) (:*:)) => source a (u b) -> target (t a) (u (t b))

(-<<-<<-) :: forall t u v category a b .
	(Traversable t category category, Covariant_ u category category, Pointable u category, Semimonoidal_ u category (:*:) (:*:), Traversable v category category)
