...
instance (Covariant (->) (->) t, Semimonoidal (->) (:*:) (:*:) t) => Semimonoidal (->) (:*:) (:*:) (Instruction t) where
	mult (Enter x :*: Enter y) = Enter $ x :*: y
	mult (Enter x :*: Instruct y) = (x :*:) <$> Instruct y
	mult (Instruct x :*: Enter y) = (:*: y) <$> Instruct x
	mult (Instruct x :*: Instruct y) = Instruct $ mult @(->) @(:*:) <$> mult (x :*: y)
