...
instance (Divariant p (->) (->) (->), Contravariant_ t (->) (->), Covariant_ u (->) (->)) => Covariant_ (t >:.:> u := p) (->) (->) where
	f -<$>- x = (f ->$<-) >-> (f -<$>-) ||= x

instance (forall i . Covariant_ (p i) (->) (->), Bivariant p (->) (->) (->), Contravariant_ t (->) (->), Contravariant_ u (->) (->)) => Contravariant_ (t >:.:< u := p) (->) (->) where
	f ->$<- x = (f ->$<-) <-> (f ->$<-) ||= x
