...
instance (forall i . Covariant (p i), Bivariant_ p (->) (->) (->), Covariant t, Covariant u) => Covariant (t <:.:> u := p) where
	f <$> x = (f <$>) -<->- (f <$>) ||= x
...
instance (forall i . Covariant (p i), Bivariant_ p (->) (->) (->), Contravariant t, Contravariant u) => Contravariant (t >:.:< u := p) where
	f >$< x = (f >$<) -<->- (f >$<) ||= x
